...
export default function Devices() {
  ...
  return (
    ...
    {electrical_devices.map((ed) => (
      <div
        key={ed.id}
        className="relative w-[250px] h-[280px] flex flex-col items-center  shadow-lg  bg-white  rounded-md p-8"
      >
        <img src={ed.img} alt="" className="w-[100px] h-[100px] " />
        <p className="text-sm text-center my-3 w-[200px] overflow-hidden text-ellipsis whitespace-nowrap">{ed.name}</p>
        <span>{currency.format(ed.price)}</span>
        <button onClick={()=>AddToCart(ed)} className='bg-sky-600 p-2 rounded-md text-white font-semibold'>أضف  للعربة</button>
        <button className=" p-1 m-1 rounded-md font-semibold absolute bottom-0"
          onClick={()=>addFavorite(ed)}
          >
            <svg
